import { RequestData, PostInfo, MediaData, MediaDetails, OutputData } from "../types/InstaAPI";
...
function getShortcode(url: string): string {
    try {
        let split_url = url.replace("https://", "").split("/");
...
async function instagramRequest(shortcode: string): Promise<RequestData> {
    try {
        ...
        let dataBody = new URLSearchParams({
...
function createOutputData(requestData: RequestData): OutputData {
    try {
        let url_list: string[] = [], media_details: MediaDetails[] = [];
        ...
        if (IS_SIDECAR) {
            ...
            media_details.push(formatMediaDetails({
                is_video: requestData.is_video,
                dimensions: { height: 0, width: 0 },
                video_url: requestData.video_url,
                display_url: requestData.display_url
            }));
...
export const instagramGetUrl = async (url_media: string): Promise<OutputData> => {
    try {
